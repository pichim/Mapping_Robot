...
Python master implementation for a **20 ms, double-transfer SPI protocol** used in a robotics control link between a **Raspberry Pi 5** (master) and an **STM32 Nucleo-F446RE** (slave, Mbed-CE). Raspberry Pi 5 with Raspberry Pi OS or Ubuntu (64-bit).
...
- **30 × 32-bit floats** per frame (**121 bytes** total)
- **CRC-8** (polynomial `0x07`, init `0x00`) for error detection
- Accurate **20 ms cycle** using `time.perf_counter()`
...
| `SPI_MSG_SIZE`        | 121     | header (1) + floats (120) + CRC (1)           |
